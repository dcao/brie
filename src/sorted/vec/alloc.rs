...
pub use core::alloc::{AllocError, Layout, LayoutError};
...
pub unsafe trait Alloc {
    ...
    ///
    /// [`handle_alloc_error`]: ../../alloc/alloc/fn.handle_alloc_error.html
    unsafe fn alloc_zeroed(&mut self, layout: Layout) -> Result<NonNull<u8>, AllocError> {
    ...
    ///
    /// [`handle_alloc_error`]: ../../alloc/alloc/fn.handle_alloc_error.html
    unsafe fn alloc_excess(&mut self, layout: Layout) -> Result<Excess, AllocError> {
    ...
    ///
    /// [`handle_alloc_error`]: ../../alloc/alloc/fn.handle_alloc_error.html
    fn alloc_one<T>(&mut self) -> Result<NonNull<T>, AllocError>
    where
        Self: Sized,
    {
        let k = Layout::new::<T>();
    ...
    ///
    /// [`handle_alloc_error`]: ../../alloc/alloc/fn.handle_alloc_error.html
    fn alloc_array<T>(&mut self, n: usize) -> Result<NonNull<T>, AllocError>
    where
        Self: Sized,
    {
    ...
    ///
    /// Always returns `Err` on arithmetic overflow.
    unsafe fn dealloc_array<T>(&mut self, ptr: NonNull<T>, n: usize) -> Result<(), AllocError>
    where
        Self: Sized,
    {
        match Layout::array::<T>(n) {
...
unsafe impl<'a> Alloc for &'a Bump {
    #[inline(always)]
    unsafe fn alloc(&mut self, layout: Layout) -> Result<NonNull<u8>, AllocError> {
        self.allocate(layout).map(|x| x.as_non_null_ptr())
    }
    ...
    unsafe fn realloc(
        ...
        if old_size == 0 {
            return self.try_alloc_layout(layout).map_err(|_| AllocError);
        }

        let new_layout = layout_from_size_align(new_size, layout.align());
        if new_size <= old_size {
            self.shrink(ptr, layout, new_layout)
                .map(|x| x.as_non_null_ptr())
        } else {
